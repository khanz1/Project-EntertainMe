...
export const TVSeason = ({ tvSeries }: TVSeasonProps) => {
  ...
  const season = useMemo(() => {
    return tvSeries.seasons.find(season => season.season_number === seasonNumber);
  }, [seasonNumber, tvSeries.seasons]);
  return (
    <Box>
      <Drawer.Root opened={isDrawerOpen} onClose={drawer.close} size="xl" position="right">
        ...
        <Drawer.Content>
          ...
          <Drawer.Body>
            {season ? <EpisodeList key={season.id} tvSeries={tvSeries} seasonNumber={season.season_number} /> : null}
          </Drawer.Body>
        </Drawer.Content>
      </Drawer.Root>
      <Box mt="lg">
